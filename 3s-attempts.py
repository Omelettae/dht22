...
READ_INTERVAL_S = 3.0        # DHT22 min ~2s between reads per sensor; extra buffer for stability
...
CSV_FIELDS = [
    ...
    "attempts_gpio17", "attempts_gpio27", "attempts_gpio22",
]
...
def read_sensor(name, results):
    ...
    dht = dht_objs[name]
    last_err = ""
    t_start = time.perf_counter()  # covers the whole read incl. all retries
    for attempt in range(1, RETRIES_PER_SENSOR + 2):  # 1-indexed attempt count
        try:
            # Force the library to bypass its internal ~2s cache cooldown
            # so every call does a real hardware read, never a stale one.
            dht._last_called = 0
    ...
    try:
        while True:
            ...
            row = {
                ...
                "attempts_gpio17": results["gpio17"]["attempts"],
                "attempts_gpio27": results["gpio27"]["attempts"],
                "attempts_gpio22": results["gpio22"]["attempts"],
            }
